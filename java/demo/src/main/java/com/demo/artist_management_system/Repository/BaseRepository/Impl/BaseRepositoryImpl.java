package com.demo.artist_management_system.Repository.BaseRepository.Impl;

import com.demo.artist_management_system.Exception.NotAcceptableException;
...
public class BaseRepositoryImpl<T extends BaseEntity, ID extends Serializable> implements IBaseRepository<T, ID> {
    ...
    public <S extends T> S update(S entity) {
        ...
        for (Field field : fields) {
            ...
            values.append(field.getName()).append(" = ").append("?,");
            try {
                params.add(field.get(entity));
            } catch (IllegalAccessException e) {
                throw new RuntimeException("Error field value for " + field.getName(), e);
